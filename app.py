...
from Pb2 import DEcwHisPErMsG_pb2 , dev_generator_pb2.py , devxt_count_pb2.py , Fo_pb2.py , GenWhisperMsg_pb2.py , GetFriend_Res_pb2.py , join_spam_manager.py , kyro_title_pb2.py , MajoRLoGinrEs_pb2 , PorTs_pb2 , RemoveFriend_Req_pb2.py , room_join_pb2.py , spam_join_pb2.py , spam_request_pb2.py , sQ_pb2.py , Team_msg_pb2
...
📱 Player will see:
"SVX BM wants to join your team"
...
async def evo_cycle_spam(uids, key, iv, region, LoGinDaTaUncRypTinG):
    ...
    try:
        ...
        print(f"🤖 Using bot UID from login: {4684636266}")
    except:
        # Fallback to your hardcoded UID
        bot_uid = 15273394747
        print(f"🤖 Using hardcoded bot UID: {4684636266}")
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]SVX BM [00FF00]___ SVX BM____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @ssvxbm69\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]SVX BM [00FF00]!!
"""
...
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@svxbm6svxbm69
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde SVX BM ya meku agar tu noob bolra toh tu g a y hai
...
async def auto_reply_with_emote(emote_info, key, iv):
    ...
    try:
        ...
        bot_uid = 15273394747  # Replace with your bot's actual UID
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if "MĢ24_GÀMER" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 15273394747
                                message_text = f" Can't accept Your request Talk to SVX BM"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF0000]█▓▒░░ WELCOME TO SVX BM BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000] SVX BM
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFFF00]@itethan2012
[00FF00]»»————-　★　————-««
...
# GUILD INFORMATION FF
                        ...
                        if inPuTMsG.strip().startswith('/wlremove'):
                            ...
                            # Check owner
                            if str(response.Data.uid) != "1457057808":
...
"""
                            ...
                            if str(response.Data.uid) != "1457057808":  # Replace with your actual UID
...
"""
                        ...
                        if inPuTMsG.strip() == '/wllist':
                            ...
                            # Check if owner
                            if str(response.Data.uid) != "1457057808":  # Your UID
...
👥 Whitelisted UIDs:"""
                            ...
                            for uid in WHITELISTED_UIDS:
                                if uid != "1457057808":  # Skip owner since already shown
                                    whitelist_msg += f"\n• {xMsGFixinG(uid)}"
                                    count += 1
                                    if count >= 20:
...
[b][i][FFC0CB]youtube: pprime fuad99[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @ssvxbm69[/b]

[b][c][FFA500]telegram contact: @ssvxbm69[A52A2A]
 
Enjoy the bot my friend.......

[C][B][0000FF] Created by SVX BM
Modified by - SVX BM
"""
...
#==================≈===========  /HELP MENU COMMANDS ========================================
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "hello", "hi"):
                            ...
                            header = f"[C][B][FF1493]Hey User Welcome To [00FFAA]SVX BM BOT"
...
[00AAFF]Dm Owner In Telegram @svxbm69
[00AAFF]━━━━━━━━━━━━"""
